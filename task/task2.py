...
class ModelWindow:
    def __init__(
        self, title, x, y, width, height, color, visibility=True, with_border=True
    ):
        self.title = title
        self.x = x
        self.y = y
    ...
    def move_horizontal(self, distance):
        ...
        if (
            new_x >= 0 and new_x + self.width <= screen_width
    ...
    def move_vertical(self, distance):
        ...
        if (
            new_y >= 0 and new_y + self.height <= screen_height
